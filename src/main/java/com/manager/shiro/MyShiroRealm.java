 ...
 * 
 * @author 吴德基  
	created at 2018年7月10日 下午8:54:22
...
public class MyShiroRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		Account userInfo = userInfoService.getAccount(username);
		System.out.println("----->>userInfo=" + userInfo);
		if (userInfo == null) {
			return null;
		}

		List<SysAccountRole> list = userInfoService.getSysAccountRole(userInfo.getId());
		List<Long> ids = new ArrayList<>();
		for (SysAccountRole rold : list) {
			ids.add(rold.getRoleId());
		}
		List<SysRole> sysRoleList = userInfoService.getSysRoleList(ids);
		for (SysRole role : sysRoleList) {
			List<Long> listPermissionIds = new ArrayList<>();
			List<SysRolePermission> listPermission = userInfoService.getSysRolePermission(role.getId());
			for (SysRolePermission per : listPermission) {
				listPermissionIds.add(per.getPermissionId());
			}
			List<SysPermission> perList = userInfoService.getSysPermissionList(listPermissionIds);
		...
		SimpleAuthenticationInfo authenticationInfo = new SimpleAuthenticationInfo(userInfo, // 用户名
				...
				ByteSource.Util.bytes(userInfo.getName()), // salt=username+salt
				getName() // realm name
		);
...
//		Object obj = principals.fromRealm(this.getClass().getName()).iterator().next();//获取session中的用户
//		System.out.println(obj);

		SimpleAuthorizationInfo authorizationInfo = new SimpleAuthorizationInfo();
		Account userInfo = (Account) principals.getPrimaryPrincipal();
